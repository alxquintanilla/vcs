...
let projectRepoName = "vcs-project-repo";
...
function setProjectRepoName (repoName) {
  projectRepoName = repoName;
}
...
function newProject (projObj) {
  ...
  if(Object.keys(projInfo).length > 1) {
...
function alignNames () {
  for(index in projectObj.files) {
    ...
    if(validateFile(file) && versionInfo.trackedFiles[file.id]) {
...
function getInfo (projectObj) {
  ...
  console.log("project Object:" + JSON.stringify(projectObj));
...
exports.setProjectRepoName = setProjectRepoName;
